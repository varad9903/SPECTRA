...
# vLLM must be imported only after CUDA_VISIBLE_DEVICES is configured (see __main__).
...
# ==============================================================================
# --- PER-IMAGE PROMPTS (6 aspects, matching DifferReID CogVLM format) ---
# ==============================================================================

# Aspect 0: Physical appearance (age, gender, build, height)
aspect_0_prompt = (
...
# Aspect 1: Facial & hair features
aspect_1_prompt = (
    "Describe this person's facial and hair features in one paragraph. "
...
# Aspect 5: Background & environment
aspect_5_prompt = (
...
def configure_cuda_devices(args):
    """Set CUDA_VISIBLE_DEVICES before torch/vLLM import. Returns tensor_parallel_size."""
    if args.mode == "multi":
    ...
    # For single mode, respect existing CUDA_VISIBLE_DEVICES if set
...
def audit_physical_gpus(args):
    """Check physical GPU health before CUDA_VISIBLE_DEVICES masks indices."""
    import subprocess
...
def verify_gpu_access(args):
    """Fail fast with a clear message if the selected GPU(s) cannot be opened."""
    import subprocess
    import torch
...
def strip_think_block(text):
    """Strip <think>...</think> reasoning blocks and clean formatting artifacts."""
    if '</think>' in text:
...
def generate_text_batch(prompts, image_path, max_tokens=256):
    """Batch-generate text for multiple prompts on the SAME image."""
...
# ==============================================================================
# --- PER-PERSON PROCESSING (descriptions only, no summaries) ---
# ==============================================================================

def process_person(pid_str, image_list, train_dir, per_image_output, dataset, max_images_per_group):
    """Generate 6-aspect descriptions per image. No summary generation."""
    if dataset == 'prcc':
        ...
        # celeb_light: no camera groups, treat all images uniformly
    ...
    for img_group, group_name in groups:
        for img_path in tqdm(img_group, desc=f"    {group_name}", leave=False):
            ...
            if rel_key in per_image_output and len(per_image_output[rel_key]) == 6:
                continue
            # Use max_tokens=1024 so Thinking models have room to finish their <think> blocks
...
def safe_save_json(filepath, new_data):
    """Re-read the file, merge new entries, then write.
    This prevents one process from overwriting the other's work."""
    existing = {}
    if os.path.exists(filepath):
        try:
            ...
            pass  # File being written by other process, use our data
...
def main():
    ...
    # When sharding, each shard writes to its own file to avoid concurrent clobber
    ...
    if args.dataset == 'prcc':
        ...
        # Celeb-light: flat directory, PID from filename
        import re as _re
        ...
        # LTCC: flat directory, PID from filename: {pid}_{clothid}_c{camid}_{seq}.png
        import re as _re
        all_images = sorted(glob.glob(os.path.join(train_dir, "*.png")))  # LTCC uses .png
...
if __name__ == "__main__":
    ...
    if args.dataset == 'prcc':
        dataset_root = "DatasetsDirectory/PRCC"
        train_dir = os.path.join(dataset_root, "rgb", "train")
    elif args.dataset == 'celeb_light':
        dataset_root = "DatasetsDirectory/Celeb-reID-light"
        train_dir = os.path.join(dataset_root, "train")
    elif args.dataset == 'ltcc':
        dataset_root = "DatasetsDirectory/LTCC_ReID"
        train_dir = os.path.join(dataset_root, "train")

    # Convert to absolute path so glob works regardless of cwd
    ...
    if not os.path.isdir(train_dir):
        raise RuntimeError(
            ...
            f"Run from the DifferReID root, e.g.: cd /mnt/data/Varad/DifferReID && python generate_captions.py ..."
        )
