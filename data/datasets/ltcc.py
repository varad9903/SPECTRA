...
class LTCC(object):
    ...
    def _process_dir_train(self, dir_path):
        ...
        for img_path in img_paths:
            pid_orig, _, camid = map(int, pattern1.search(img_path).groups())
            clothes = pattern2.search(img_path).group(1)
            camid -= 1 # index starts from 0

            pid = pid2label[pid_orig]
            ...
            if self.load_sum_ft and 0 in self.bio_index:
                # Use original PID to look up biometric summary
                # generate_summaries saves keys as str(int(pid_str)), e.g. "0", "2", "10"
                pid_key = str(pid_orig)
                id_ft=np.asarray(sum_id_info[pid_key][self.ft_name])
