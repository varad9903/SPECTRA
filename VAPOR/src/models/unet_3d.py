...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(self, '_freeu_params') and self._freeu_params:
                fp = self._freeu_params
                if i == 0:
                    s_val, b_val = fp['s1'], fp['b1']
                elif i == 1:
                    s_val, b_val = fp['s2'], fp['b2']
                else:
                    s_val, b_val = 1.0, 1.0
                ...
                if s_val != 1.0:
                    ...
                    for r in res_samples:
                        orig_shape = r.shape
                        if r.ndim == 5:
                            r_2d = r.reshape(-1, r.shape[-2], r.shape[-1])
                        else:
                            r_2d = r.reshape(-1, r.shape[-2], r.shape[-1])
                        r_freq = torch.fft.fftn(r_2d, dim=(-2, -1))
                        r_freq = r_freq * s_val
                        r_2d = torch.fft.ifftn(r_freq, dim=(-2, -1)).real
                        r = r_2d.reshape(orig_shape)
                        new_res.append(r)
                    res_samples = tuple(new_res)
