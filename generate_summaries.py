"""
generate_summaries.py — Step 2 of 2

Reads the per-image descriptions JSON (produced by generate_captions.py)
and generates biometric + clothing summary JSON files using Qwen3-8B.

Uses Qwen3-8B (text-only, 8B params) with thinking DISABLED for fast,
clean summaries without any reasoning text leakage.

Usage:
    python generate_summaries.py --mode single --gpu_id 1
    python generate_summaries.py --mode single --gpu_id 1 --num_persons 5
    python generate_summaries.py --mode multi --visible_gpus "1,3"
"""
...
def generate_summary(prompt_text, max_tokens=512):
    """Generate a text-only summary using Qwen3-8B with thinking DISABLED.

    With enable_thinking=False on Qwen3-8B:
      - No <think>...</think> block is generated at all
      - Output is directly the clean summary
      - Much faster than thinking mode
    """
    messages = [
    ...
    # enable_thinking=False: clean output, no reasoning tags
    ...
    # Clean any stray special tokens (shouldn't happen but just in case)
...
def get_person_ids_from_descriptions(per_image_output, dataset):
    """Extract unique person IDs from per-image description keys."""
    pids = set()
...
def get_bio_descriptions_for_person(pid, per_image_output, dataset):
    """Extract bio descriptions (aspect 0) for a person across all images."""
    bio_descs = []
    for key, aspects in per_image_output.items():
        ...
        if len(aspects) < 1:
            continue
        bio_text = aspects[0]  # Aspect 0: physical appearance
        if bio_text:
...
def process_person_summaries(pid_str, per_image_output, max_sum, dataset):
    """Generate biometric summary for a single person."""
...
if __name__ == "__main__":
    ...
    # Qwen3-8B is text-only — use AutoTokenizer (not AutoProcessor)
    ...
    llm = LLM(
        ...
        max_model_len=16384,    # Summaries don't need 32K context
        enforce_eager=True,
