...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path=self.dataset[index]['image_path']   
        
        img = read_image(img_path)
        if self.transform is not None:
            img = self.transform(img)
        if isinstance(img, tuple):
            self.dataset[index]['image']=img[0]
            self.dataset[index]['augmentation_descriptions']=img[1]
        else:
            self.dataset[index]['image']=img
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data={'image':clip,
              ...
              'clothes_id':self.dataset[index]['clothes_id'],
