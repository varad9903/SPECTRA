...
class VisionTransformer(nn.Module):
    def __init__(self, config:dict, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, 
                 clip_output_dim: int=-1,num_classes: int=-1,head_init_scale: float = 0.001,
                 cameraNum: int = 0, last_layer: str = 'clipFc_clsFc', bioClip=False, add_nonBio: bool = False, nonBio_num: int = 3, reverse_bio: bool = False):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor,camera_id):
        x = self.conv1(x)
...
if __name__ == "__main__":
    ...
    x = preprocess(Image.open("/path/to/workspace/MADE/paddimg.png")).unsqueeze(0)
    cloth_id = torch.tensor([1])
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    config_file='/path/to/workspace/MADE/configs/ltcc/clipViTB_bio.yml'
    ...
    writer = SummaryWriter('/path/to/workspace/Simple-CCReID/logs/model/resnetClip')
