...
from utils.nfc import NFC, AdaptiveNFC
...
class R1_mAP_eval():
    def __init__(self, num_query, num_gallery=0,max_rank=50, feat_norm=True, reranking=False,logResults=False,ignore_cam=False,gallery_first=False, nfc_mode='none', nfc_k1=2, nfc_k2=2, nfc_alpha=0.7, nfc_max_k=20, nfc_apply_to='both'):
        ...
        self.nfc_mode = nfc_mode
        self.nfc_k1 = nfc_k1
        self.nfc_k2 = nfc_k2
        self.nfc_alpha = nfc_alpha
        self.nfc_max_k = nfc_max_k
        self.nfc_apply_to = nfc_apply_to
    ...
    def compute(self):  # called after each epoch
        ...
        if self.reranking:
            ...
            # Apply NFC before distance computation
            if self.nfc_mode == 'adaptive':
                apply_q = self.nfc_apply_to in ('both', 'query_only')
                apply_g = self.nfc_apply_to in ('both', 'gallery_only')
                print(f'=> Applying AdaptiveNFC (alpha={self.nfc_alpha}, max_k={self.nfc_max_k}, apply_to={self.nfc_apply_to})')
                if apply_q:
                    print(f'   Query [{qf.shape[0]}]:')
                    qf = AdaptiveNFC(qf, max_k=self.nfc_max_k, alpha=self.nfc_alpha)
                    qf = torch.nn.functional.normalize(qf, dim=1, p=2)
                if apply_g:
                    print(f'   Gallery [{gf.shape[0]}]:')
                    gf = AdaptiveNFC(gf, max_k=self.nfc_max_k, alpha=self.nfc_alpha)
                    gf = torch.nn.functional.normalize(gf, dim=1, p=2)
            elif self.nfc_mode == 'fixed':
                apply_q = self.nfc_apply_to in ('both', 'query_only')
                apply_g = self.nfc_apply_to in ('both', 'gallery_only')
                print(f'=> Applying Fixed NFC (k1={self.nfc_k1}, k2={self.nfc_k2}, apply_to={self.nfc_apply_to})')
                if apply_q:
                    qf = NFC(qf, k1=self.nfc_k1, k2=self.nfc_k2)
                    qf = torch.nn.functional.normalize(qf, dim=1, p=2)
                if apply_g:
                    gf = NFC(gf, k1=self.nfc_k1, k2=self.nfc_k2)
                    gf = torch.nn.functional.normalize(gf, dim=1, p=2)
...
class R1_mAP_eval_LTCC():
    def __init__(self, num_query, num_gallery=0, max_rank=50, feat_norm=True, reranking=False,gallery_first=False, nfc_mode='none', nfc_k1=2, nfc_k2=2, nfc_alpha=0.7, nfc_max_k=20, nfc_apply_to='both'):
