...
class ProjectionMLP(nn.Module):
    """2-layer MLP projection head (replaces nn.Linear for richer mapping capacity)."""
...
class CrossAttentionRefiner(nn.Module):
    """Cross-Attention Feature Refiner — learned replacement for test-time ANFC.
    
    For each input feature, computes cross-attention against a learnable memory
    bank of identity prototypes. This is the training-time equivalent of NFC:
    - NFC: average with nearest neighbors (test-time, multi-sample)
    - This: attend to learned prototypes (train+test time, single-sample)
    
    Key design choices:
    1. Learnable nn.Parameter prototypes — trained end-to-end via CE loss
       backprop, naturally converging to identity centroids (unlike PACR's
       EMA prototypes which absorb early clothing bias)
    2. Gated residual — initialized to zero so training starts from baseline.
       The gate gradually opens as prototypes mature via backprop.
    3. Multi-head attention — captures different aspects of identity similarity
    4. Single-image operation — no multi-sample aggregation needed
    
    At test time on unseen identities: the attention pattern still works because
    prototypes act as a learned feature codebook (not identity-specific lookup).
    A test feature attends to the most structurally similar prototypes.
    """
    
    def __init__(self, embed_dim=1024, num_heads=8, num_prototypes=150, dropout=0.1):
        super().__init__()
        self.num_prototypes = num_prototypes
        self.embed_dim = embed_dim
        
        # Learnable prototype memory bank
        self.memory_bank = nn.Parameter(torch.empty(num_prototypes, embed_dim))
        ...
        # Gated residual: starts at 0 → identity function initially
        # tanh(0) = 0, so feat_refined = LN(feat_bio + 0) at start
        # The model learns to open the gate as prototypes improve via backprop
        self.gate = nn.Parameter(torch.zeros(1))
    
    def forward(self, feat):
        """Refine features by attending to prototype memory bank.
        
        Args:
            feat: [B, D] feature vector (feat_bio from backbone)
        Returns:
            feat_refined: [B, D] refined feature vector
        """
        B = feat.size(0)
        
        # Query: feat as single-token sequence [B, 1, D]
        query = feat.unsqueeze(1)
        
        # Key/Value: memory bank expanded for batch [B, num_prototypes, D]
        # .to(dtype=) handles AMP mixed precision (float16 features, float32 params)
        memory = self.memory_bank.unsqueeze(0).expand(B, -1, -1).to(dtype=feat.dtype)
        
        # Cross-attention: feat attends to all prototypes
        attn_output, _ = self.cross_attn(query, memory, memory)  # [B, 1, D]
        attn_output = attn_output.squeeze(1)  # [B, D]
        
        # Gated residual + LayerNorm
        gate_value = self.gate.tanh()  # range [-1, 1], starts at 0
        feat_refined = self.norm(feat + gate_value * attn_output)
...
class Eva(nn.Module):
    ...
    def __init__(
        ...
        self.use_projection_mlp=use_projection_mlp
        ...
        if self.last_layer=='fc':
            ...
            if self.last_layer=='clipFc':
                # Toggle between MLP and Linear via config
                if self.use_projection_mlp:
                    self.head_clip_bio = ProjectionMLP(embed_dim, clip_feat_dim) if clip_feat_dim > 0 else nn.Identity()
                else:
                    self.head_clip_bio = nn.Linear(embed_dim, clip_feat_dim, bias=False) if clip_feat_dim > 0 else nn.Identity()
        ...
        # Cross-Attention Feature Refiner (learned ANFC replacement)
        if self.use_refiner:
            self.refiner = CrossAttentionRefiner(
                embed_dim=embed_dim,
                num_heads=refiner_heads,
                num_prototypes=num_classes,
                dropout=refiner_dropout
            )
            print(f"CrossAttentionRefiner enabled: {num_classes} prototypes, "
                  f"{refiner_heads} heads, dropout={refiner_dropout}")

        self.apply(self._init_weights)

        # Ensure gate starts at zero after weight init (identity function at start)
        if self.use_refiner:
            nn.init.zeros_(self.refiner.gate)
            nn.init.xavier_uniform_(self.refiner.memory_bank)
    ...
    def forward(self, x,cam_label=3,norm=False):
        ...
        # Apply cross-attention refinement if enabled
        if self.use_refiner:
            feat_refined = self.refiner(feat_bio)
        else:
            feat_refined = feat_bio
            
        if not self.training:
            # At test time: return refined features (replaces ANFC)
            return feat_refined
        else:
            # Triplet loss operates on refined features
            feat_ouput=[feat_refined]
            
            # Dual CE: classify both raw and refined features
            # This ensures the backbone keeps strong classification while
            # the refiner also learns to produce classifiable features
            if self.use_refiner:
                cls_score_bio = self.head(feat_bio)
                cls_score_refined = self.head(feat_refined)
                score_output={'cls_score': [cls_score_bio, cls_score_refined]}
            else:
                cls_score = self.head(feat_bio)
                score_output={'cls_score':cls_score}
            
            if self.reverse_bio:
                # GRL stays on raw feat_bio — clothing suppression on backbone output
            ...
            if self.last_layer in ['clipFc','clipMLP']:
                # CLIP ITC on refined features (alignment on the test-time feature)
                clip_bio_score=self.head_clip_bio(feat_refined)
                ...
                # Expose projected features for orthogonality loss
                score_output['bio_projected']=clip_bio_score.detach()  # detached for ortho target
...
def eva02_large_patch14_clip_224_bio(pretrained=False,config={}, **kwargs) -> Eva:
    ...
    model_args = dict(
        ...
        use_projection_mlp=config.MODEL.USE_PROJECTION_MLP,
        use_refiner=getattr(config.MODEL, 'USE_REFINER', False),
        refiner_heads=getattr(config.MODEL, 'REFINER_HEADS', 8),
        refiner_dropout=getattr(config.MODEL, 'REFINER_DROPOUT', 0.1),
    )
