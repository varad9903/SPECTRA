...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, cfg):
        ...
        # Handle NOBIO_INDEX / BIO_INDEX as either list (new) or string (legacy)
        nbi = cfg.DATA.NOBIO_INDEX
        nonBio_num = len(nbi) if isinstance(nbi, (list, tuple)) else len(nbi.strip('*').split('*'))
        bi = cfg.DATA.BIO_INDEX
        bio_num = len(bi) if isinstance(bi, (list, tuple)) else len(bi.strip('*').split('*'))
